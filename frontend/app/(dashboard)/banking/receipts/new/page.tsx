...
import { createReceipt } from "@/features/banking/api/banking-api";
...
export default function NewReceiptPage() {
  ...
  useEffect(() => {
    ...
    apiClient.get("/api/v1/billing/invoices").then(res => {
       setInvoices(res.data.data.filter((i: any) => i.document_type === 'SALES' && i.status === 'POSTED'));
    });
  }, [fetchMasters]);
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      <h1 className="text-3xl font-bold mb-8">New Receipt Voucher</h1>
          ...
          <Card>
            <CardContent className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                ...
                <div className="space-y-2">
                  <Label>Received From (Party Ledger)</Label>
              ...
              <div className="space-y-2 mb-8">
                ...
                <Input placeholder="Receipt details..." {...methods.register("narration")} />
              </div>

              <div className="flex justify-end gap-4 border-t pt-4">
                <Button variant="outline" type="button" onClick={() => router.back()}>Cancel</Button>
                <Button type="submit" disabled={loading}>Post Receipt</Button>
              </div>
            </CardContent>
          </Card>
        </form>
